# File Path: app.py
# Description: Streamlit WebGIS application for Ocean PFZ mapping with coupled weighting and safe NetCDF file handling.
...
# ۱. اسلایدر یکپارچه وزن SST
sst_weight = st.sidebar.slider("وزن جبهه‌های حرارتی SST", 0.0, 1.0, 0.6, 0.05)
# ۲. محاسبه خودکار وزن کلروفیل به عنوان متمم SST
...
def generate_fronts_fallback(nc_path, output_geojson_path, user_threshold):
    try:
        ...
        # بستن خودکار فایل جهت جلوگیری از خطای Permission Denied
        ...
        def extract_lines(t_val):
            ...
            for segs in cs.allsegs:
                for poly in segs:
                    if len(poly) > 2:  # فیلتر خطوط کوتاه
        ...
        if lines:
            ...
            gdf_fronts = gdf_fronts[gdf_fronts['Length_km'] > 1.0] # حداقل طول ۱ کیلومتر
            ...
            if not gdf_fronts.empty:
                ...
                record_error("خطوط جبهه یافت شدند اما همگی کوتاه‌تر از ۱.۰ کیلومتر بوده و فیلتر شدند.")
                return False
        else:
